...
import { getProductSocialNetworkById } from 'apiSdk/product-social-networks';
...
function ProductSocialNetworkViewPage() {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Product Social Network Detail View
          </Text>
          {hasAccess('product_social_network', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/product-social-networks/edit/${data?.id}`} passHref legacyBehavior>
              <Button
            ...
            <Box>
              ...
              {hasAccess('social_network', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/social-networks/view/${data?.social_network?.id}`}>
                      {data?.social_network?.name}
                    </Link>
                  </Text>
                </Flex>
              )}
