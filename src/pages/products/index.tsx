...
function ProductListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProductInterface[]>(
    ...
    () =>
      getProducts({
        relations: ['organization', 'product_delivery_company.count', 'product_social_network.count'],
      }),
  );
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('product_social_network', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.product_social_network}</Td>
                    )}
                    <Td>
